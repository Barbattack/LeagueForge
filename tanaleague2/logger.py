"""
TanaLeague - Logging Configuration
==================================
...
FILE LOG:
    - logs/tanaleague.log      - Log principale (INFO+)
    - logs/tanaleague_debug.log - Log dettagliato (DEBUG+, solo se DEBUG=True)
...
# Directory logs (relativa a tanaleague2/)
...
def setup_logging():
    ...
    # Root logger
    root_logger = logging.getLogger("tanaleague")
...
def get_logger(name: str = None) -> logging.Logger:
    ...
    Args:
        name: Nome del modulo (usa __name__ per auto-naming)
              Es: "tanaleague.app", "tanaleague.achievements"
    ...
    # Prefissa con "tanaleague." se non presente
    if name:
        if not name.startswith("tanaleague"):
            name = f"tanaleague.{name}"
    else:
        name = "tanaleague"
